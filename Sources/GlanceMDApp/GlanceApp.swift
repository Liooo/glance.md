...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func showSelectedMarkdown(reason: TriggerReason) {
        ...
        guard let text = selectionReader.selectedText(),
            ...
            previewPanel?.showMessage("No text selected", near: reason.location)
            return
        }
...
final class PreferencesStore {
    private enum Key {
        ...
        static let previewSizingMode = "previewSizingMode"
        ...
        static let fitPreviewMaxWidth = "fitPreviewMaxWidth"
        static let fitPreviewMaxHeight = "fitPreviewMaxHeight"
    }

    private enum Default {
        static let fitPreviewMaxWidth: CGFloat = 900
        static let fitPreviewMaxHeight: CGFloat = 700
        static let previewSizingMode = PreviewSizingMode.fitToContent
    }
    ...
    var rememberPreviewSize: Bool {
        get {
            previewSizingMode == .rememberPreviousSize
        }
        set {
            previewSizingMode = newValue ? .rememberPreviousSize : .fitToContent
        }
    }

    var previewSizingMode: PreviewSizingMode {
        get {
            if let rawValue = UserDefaults.standard.string(forKey: Key.previewSizingMode),
                let mode = PreviewSizingMode(rawValue: rawValue)
            {
                return mode
            }

            return Default.previewSizingMode
        }
        set {
            UserDefaults.standard.set(newValue.rawValue, forKey: Key.previewSizingMode)
            UserDefaults.standard.set(
                newValue == .rememberPreviousSize, forKey: Key.rememberPreviewSize)
            if newValue != .rememberPreviousSize {
                previewSize = nil
            }
        }
    }
    ...
    var fitPreviewMaxSize: NSSize {
        get {
            let width = UserDefaults.standard.double(forKey: Key.fitPreviewMaxWidth)
            let height = UserDefaults.standard.double(forKey: Key.fitPreviewMaxHeight)
            return NSSize(
                width: width > 0 ? width : Default.fitPreviewMaxWidth,
                height: height > 0 ? height : Default.fitPreviewMaxHeight
            )
        }
        set {
            UserDefaults.standard.set(max(newValue.width, 1), forKey: Key.fitPreviewMaxWidth)
            UserDefaults.standard.set(max(newValue.height, 1), forKey: Key.fitPreviewMaxHeight)
        }
    }
...
enum PreviewSizingMode: String, CaseIterable, Identifiable {
    case rememberPreviousSize
    case fitToContent = "fitWidthToContent"
...
final class SettingsViewModel: ObservableObject {
    ...
    @Published var previewSizingMode: PreviewSizingMode {
        didSet { preferences.previewSizingMode = previewSizingMode }
    }
    @Published var fitPreviewMaxWidth: Double {
        didSet { saveFitPreviewMaxSize() }
    }
    @Published var fitPreviewMaxHeight: Double {
        didSet { saveFitPreviewMaxSize() }
    }
    ...
    init(preferences: PreferencesStore) {
        ...
        previewSizingMode = preferences.previewSizingMode
        fitPreviewMaxWidth = preferences.fitPreviewMaxSize.width
        fitPreviewMaxHeight = preferences.fitPreviewMaxSize.height
    ...
    private func saveFitPreviewMaxSize() {
        preferences.fitPreviewMaxSize = NSSize(
            width: fitPreviewMaxWidth,
            height: fitPreviewMaxHeight
        )
    }
...
struct SettingsView: View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Picker("Preview sizing", selection: $viewModel.previewSizingMode) {
                        ForEach(PreviewSizingMode.allCases) { mode in
                            Text(mode.title)
                                .padding(.bottom, mode == .rememberPreviousSize ? 3 : 0)
                                .tag(mode)
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        previewMaxSizeField(
                            title: "max width:",
                            value: $viewModel.fitPreviewMaxWidth
                        )
                        previewMaxSizeField(
                            title: "max height:",
                            value: $viewModel.fitPreviewMaxHeight
                        )
                    }
                    .padding(.leading, 28)
                    .disabled(viewModel.previewSizingMode != .fitToContent)
                    .opacity(viewModel.previewSizingMode == .fitToContent ? 1 : 0.45)
                }
            }
        }
        .padding()
    ...
    private func previewMaxSizeField(title: String, value: Binding<Double>) -> some View {
        HStack(spacing: 0) {
            Text(title)
                .font(.subheadline)
                .foregroundColor(.secondary)
                .frame(width: 68, alignment: .leading)
            TextField(title, value: value, formatter: Self.pixelFormatter)
    ...
    private static let pixelFormatter: NumberFormatter = {
    ...
    WKScriptMessageHandler
{
    ...
    private var currentPreviewAnchorPoint: CGPoint?
    ...
    func show(markdown: String, near point: CGPoint) {
        ...
        currentPreviewAnchorPoint = point
        previewLoadToken += 1
        ...
        applyDocumentTheme()
        if preferences.previewSizingMode != .fitToContent {
    ...
    func webView(_: WKWebView, didFinish _: WKNavigation!) {
        applyDocumentTheme()
        let shouldRevealAfterFit = currentMarkdown != nil
            && preferences.previewSizingMode == .fitToContent
            && !panel.isVisible
        fitPanelToContentIfNeeded(revealAfterFit: shouldRevealAfterFit, token: previewLoadToken)
    ...
    func windowDidResize(_: Notification) {
        ...
        if shouldRememberCurrentPanelSize,
            preferences.previewSizingMode == .rememberPreviousSize
        {
    ...
    private func showPanel(near point: CGPoint, size: NSSize = PreviewPanelController.defaultSize) {
        ...
        presentPanel(ensuring: frame)
    }

    private func presentPanel(ensuring frame: NSRect) {
    ...
    private func renderMermaid(token: Int) {
        ...
        ensureMermaidLoaded(token: token) { [weak self] in
            ...
            self.webView.evaluateJavaScript(
                ...
                self.fitPanelToContentIfNeeded(
                    revealAfterFit: false, token: self.previewLoadToken)
            }
        }
    }
    ...
    private func fitPanelToContentIfNeeded(revealAfterFit: Bool, token: Int) {
        guard currentMarkdown != nil,
            preferences.previewSizingMode == .fitToContent
        else {
            return
        }

        let maxSize = maximumFitPreviewSize(near: currentPreviewAnchorPoint)
        webView.evaluateJavaScript(Self.contentSizeMeasurementScript(maxWidth: maxSize.width)) {
            [weak self] result, error in
            DispatchQueue.main.async {
                guard let self,
                    self.currentMarkdown != nil,
                    self.previewLoadToken == token,
                    self.preferences.previewSizingMode == .fitToContent
                else {
                    return
                }

                if let error {
                    appLog("failed to measure preview content size: \(error)")
                    if revealAfterFit, let point = self.currentPreviewAnchorPoint {
                        self.showPanel(near: point, size: self.preferredPreviewSize)
                    }
                    return
                }

                guard let contentSize = Self.contentSize(from: result),
                ...
                let size = self.clampedFitPreviewSize(
                    contentSize,
                    near: self.currentPreviewAnchorPoint
                )
                let frame = NSRect(
                    origin: self.clampedOrigin(
                        near: self.currentPreviewAnchorPoint ?? NSEvent.mouseLocation,
                        size: size
                    ),
                    size: size
                )

                guard revealAfterFit
                    || abs(frame.width - self.panel.frame.width) >= 1
                    || abs(frame.height - self.panel.frame.height) >= 1
                else {
                    return
                }
                ...
                self.applyPanelFrame(frame)
                if revealAfterFit {
                    self.presentPanel(ensuring: frame)
                }
            }
        }
    }

    private static func contentSize(from value: Any?) -> NSSize? {
        guard let value = value as? [String: Any],
            let width = cgFloat(from: value["width"]),
            let height = cgFloat(from: value["height"])
        else {
            return nil
        }
        return NSSize(width: width, height: height)
    ...
    private static func contentSizeMeasurementScript(maxWidth: CGFloat) -> String {
        let encodedMaxWidth = String(
            data: try! JSONEncoder().encode(maxWidth),
            encoding: .utf8
        ) ?? "0"

        return """
        (() => {
          const maxWidth = \(encodedMaxWidth);
          ...
          clone.style.boxSizing = bodyStyle.boxSizing;
          clone.style.padding = bodyStyle.padding;
          clone.style.font = bodyStyle.font;
          clone.style.lineHeight = bodyStyle.lineHeight;

          clone.querySelectorAll('table').forEach((table) => {
            table.style.width = 'max-content';
          });
          clone.querySelectorAll('pre, .qm-table-wrap, .qm-mermaid').forEach((element) => {
          ...
          const width = Math.max(1, Math.min(naturalWidth + 2, maxWidth));
    ...
    private var preferredPreviewSize: NSSize {
        guard preferences.previewSizingMode == .rememberPreviousSize,
            let size = preferences.previewSize
        else {
    ...
    private func configureMouseMonitors() {
        ...
        localMouseMonitor = NSEvent.addLocalMonitorForEvents(matching: mouseEvents) {
            [weak self] event in
            self?.closeIfClickIsOutsidePreview(at: NSEvent.mouseLocation)
            return event
        }
        globalMouseMonitor = NSEvent.addGlobalMonitorForEvents(matching: mouseEvents) {
            ...
            DispatchQueue.main.async {
                self?.closeIfClickIsOutsidePreview(at: screenPoint)
            }
        }
    }

    private func closeIfClickIsOutsidePreview(at point: CGPoint) {
        guard panel.isVisible, !containsScreenPoint(point) else {
            return
        }
    ...
    private func closeWithoutRestoringFocus() {
        guard panel.isVisible else {
            return
        }
    ...
    private func maximumFitPreviewSize(near point: CGPoint?) -> NSSize {
        let screenPoint = point ?? NSEvent.mouseLocation
        let screen = NSScreen.screens.first { $0.frame.contains(screenPoint) } ?? NSScreen.main
        let visible = screen?.visibleFrame ?? NSRect(x: 0, y: 0, width: 1440, height: 900)
        let padding: CGFloat = 10
        let preferenceMaxSize = preferences.fitPreviewMaxSize

        return NSSize(
            width: min(
                max(preferenceMaxSize.width, panel.minSize.width),
                max(panel.minSize.width, visible.width - padding * 2)
            ),
            height: min(
                max(preferenceMaxSize.height, panel.minSize.height),
                max(panel.minSize.height, visible.height - padding * 2)
            )
        )
    }

    private func clampedFitPreviewSize(_ size: NSSize, near point: CGPoint?) -> NSSize {
        let maximumSize = maximumFitPreviewSize(near: point)
        return NSSize(
            width: min(max(size.width, panel.minSize.width), maximumSize.width),
            height: min(max(size.height, panel.minSize.height), maximumSize.height)
        )
    }
